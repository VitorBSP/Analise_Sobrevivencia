...
prob_sem_inter <- function(obj, filter){
  obj <- obj |> filter(n.event != 0) |> select(estimate, time) |> 
  y = obj |> filter(time == temp) |> select(estimate)
  list(prob = y, inter = y_)
}
...
```{r}
kaplan1 = survfit(Surv(df1$leucemia,df1$censura)~grupos) |> 
    broom::tidy()
nelson1 = survfit(Surv(df1$leucemia,df1$censura)~grupos, stype = 2) |> 
    broom::tidy()
```
...
nquimio <- kaplan1 |> 
    ...
    ggplot()+
    geom_line(data = quimio, aes(Tempo, Estimate_KM, color = 'Kaplan-Meier - Mantido'), size = 2)+
    geom_point(data = quimio,aes(Tempo, Estimate_KM), color = 'red', size = 3) +
    geom_line(data = quimio,aes(Tempo, Estimate_NH, color = 'Nelson-Aalen - Mantido'), size = 2)+
    geom_point(data = quimio,aes(Tempo, Estimate_NH), color = 'blue', size = 3) +
    geom_line(data = nquimio, aes(Tempo, Estimate_KM, color = 'Kaplan-Meier - Não Mantido'), size = 2)+
    geom_point(data = nquimio,aes(Tempo, Estimate_KM), color = 'pink', size = 3) +
    geom_line(data = nquimio,aes(Tempo, Estimate_NH, color = 'Nelson-Aalen - Não Mantido'), size = 2)+
...
```{r}
prob(kaplan1 |> filter(strata == 'grupos=1'), 30) 
prob(kaplan1 |> filter(strata == 'grupos=2'), 30)
prob(nelson1 |> filter(strata == 'grupos=1'), 30) 
prob(nelson1 |> filter(strata == 'grupos=2'), 30)
```
